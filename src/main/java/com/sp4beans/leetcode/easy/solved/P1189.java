package com.sp4beans.leetcode.easy.solved;

//        给你一个字符串 text，你需要使用 text 中的字母来拼凑尽可能多的单词 "balloon"（气球）。
//
//        字符串 text 中的每个字母最多只能被使用一次。请你返回最多可以拼凑出多少个单词 "balloon"。
//
//         
//
//        示例 1：
//
//
//
//        输入：text = "nlaebolko"
//        输出：1
//        示例 2：
//
//
//
//        输入：text = "loonbalxballpoon"
//        输出：2
//        示例 3：
//
//        输入：text = "leetcode"
//        输出：0
//         
//
//        提示：
//
//        1 <= text.length <= 10^4
//        text 全部由小写英文字母组成

public class P1189 {

    private static class Solution {
        public int maxNumberOfBalloons(String text) {
            int[] wordCount = new int[26];
            char[] cc = text.toCharArray();
            for (char c : cc) {
                wordCount[c - 'a'] ++;
            }

            int ans = Integer.MAX_VALUE;
            ans = Math.min(ans, wordCount['b' - 'a']);
            ans = Math.min(ans, wordCount['a' - 'a']);
            ans = Math.min(ans, wordCount['n' - 'a']);
            ans = Math.min(ans, wordCount['l' - 'a'] / 2);
            ans = Math.min(ans, wordCount['o' - 'a'] / 2);
            return ans;
        }
    }

    public static void main(String[] args) {
        Solution solution = new Solution();
        System.out.println(solution.maxNumberOfBalloons("nlaebolko"));
        System.out.println(solution.maxNumberOfBalloons("loonbalxballpoon"));
        System.out.println(solution.maxNumberOfBalloons("leetcode"));
    }
}
